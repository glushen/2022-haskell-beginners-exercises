...
subString start end str = take (realEnd - realStart + 1) (drop realStart str)
    where realStart = max 0 start
          realEnd = max (start - 1) end
...
lowerAndGreater n list = (show n)
    ++ " is greater than " ++ (show greaterCount)
    ++ " elements and lower than " ++ (show lowerCount) ++ " elements"
    where
        countLowerAndGreater :: [Int] -> (Int, Int)
        countLowerAndGreater _list
            | null _list = (0, 0)
            | current < n = (l, g + 1)
            | current > n = (l + 1, g)
            | otherwise = (l, g)
            where
                (l, g) = countLowerAndGreater (tail _list)
                current = head _list
        (lowerCount, greaterCount) = countLowerAndGreater list
